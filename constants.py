...
\noindent \textbf{Class Activation Map:} Originally proposed for explaining the decision-making process of deep-learning classifier in computer vision, the Class Activation Map (CAM)~\cite{cam} enables the highlighting of the parts of an image that contributed the most to obtaining the predicted label. 
A one-dimensional adaptation of this method was proposed for time series classification to highlight the relevant subsequences of a time series~\cite{Wang2016TimeSC, 10.1007/s10618-019-00619-1, dcam}.
...
\noindent \textbf{Attention Map:} With the advent of Transformer models in numerous domains, explainability methods have been proposed to visualize the insight of the self-attention mechanism and localize important features~\cite{attentionmap}.
